...
#if defined(NDEBUG)
# error "diot cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "diot Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // diot: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// diot: find block wanted by given orphan block
...
// diot: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal diot miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // diot: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // diot: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // diot: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // diot: track money supply and mint amount info
...
// diot: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // diot: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // diot: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // diot: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// diot: attempt to generate suitable proof-of-stake
